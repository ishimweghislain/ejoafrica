                    ...
                    <div className="w-full space-y-6 md:space-y-10 animate-in zoom-in duration-300 pb-10">
                        <div className="space-y-4 md:space-y-6">
                            <div className="flex items-center justify-between">
                                ...
                                <div className={`flex items-center gap-2 font-black text-[10px] uppercase tracking-widest px-4 py-1.5 rounded-xl border shadow-sm transition-all ${timeLeft !== null && timeLeft <= 5 ? 'bg-rose-600 text-white border-rose-600 animate-pulse' : 'bg-white text-slate-400 border-slate-100'}`}>
                                    <Clock className={`w-4 h-4 ${timeLeft !== null && timeLeft <= 5 ? 'text-white' : 'text-rose-500'}`} />
                                    <span>{timeLeft !== null ? `${timeLeft}s Left` : `${currentQuestion.timer}s Limit`}</span>
                                </div>
                            </div>
